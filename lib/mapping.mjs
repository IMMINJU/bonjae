...
//   가지 = 빛을 향한 옆 역량  곁다리로 키운 것들 (굴광성)
//   열매 = 외부 검증된 성과   남들이 인정한 결과물
//   뿌리 = 따라온 깊이        파고든 전문 영역
//   빛   = 향해 자라는 프레임  환경/조직/시장
//
...
export function mapProfileToBonsai(profile) {
  ...
  // 발산도(divergence): 직무·활동이 서로 얼마나 다른가 → 줄기 휨·가지 펼침의 "양".
  // (예전 spread[wide/balanced/upright]를 대체. divergence가 형태를 직접 정한다.)
  const divergence = clamp(character.divergence ?? 5, 1, 10);
  ...
  return {
    ...
    fruits: mappedFruits,
    light,
    lightStrength: clamp(lightStrength ?? 5, 1, 10), // 1~10 (빛 강도·뻗음)

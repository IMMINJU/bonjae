...
// 생애 단계 — 나무 전체의 크기·굵기·수령. 주니어/시니어 크기 차이를 만든다.
const MATURITY = {
  sapling:
    "MATURITY: This is a YOUNG, SMALL bonsai sapling — early in its life. " +
    "It is noticeably SMALL and delicate: a SLENDER, THIN trunk (smooth young bark, no gnarls), " +
    "only a few short, thin branches, and a sparse, light canopy with few leaves. " +
    "The little tree occupies only the lower-center of the pot and leaves a LOT of empty space and air around and above it. " +
    "Keep the pot modest. The whole tree reads as small, fresh, and still growing.",
  young:
    "MATURITY: This is a MID-LIFE bonsai of moderate size — established but still growing. " +
    "A medium-thickness trunk with some character, a moderate spread of branches, and a fuller but not dense canopy. " +
    "It fills a fair part of the pot, with some open space remaining.",
  old:
    "MATURITY: This is an OLD, LARGE, MAJESTIC bonsai with decades of growth. " +
    "A THICK, STURDY, gnarled trunk with rugged aged bark, heavy spreading branches, and a full, dense canopy. " +
    "Thick mature roots flare powerfully over the soil (strong nebari). " +
    "The grand tree FILLS the pot and frame, commanding and well-developed — clearly an ancient, senior tree.",
};
...
const PART_NAMES = {
  ko: { fruit: "열매", branch: "가지", trunk: "줄기", root: "뿌리", light: "빛" },
  en: { fruit: "Fruit", branch: "Branch", trunk: "Trunk", root: "Root", light: "Light" },
};
...
export function buildPrompt(spec, { style = "ink", lang = "ko" } = {}) {
  ...
  const branchDesc =
    `THE BRANCHES (lateral competencies grown toward the light), each one clearly attached to the trunk: ` +
    branchLines.join("; ") + ".";
  ...
  let lightDesc = "";
  if (spec.light) {
    const lightLabel = labelText("light", spec.light, lang);
    labels.push(lightLabel);
    const ls = spec.lightStrength ?? 5;
    const beam =
      ls >= 8
        ? "STRONG, radiant sunlight pours from the upper corner and reaches far across the whole tree, the foliage clearly straining toward it"
        : ls >= 5
        ? "warm sunlight comes from the upper corner"
        : "a soft, faint glow sits in the upper corner";
    lightDesc =
      `${beam} (the frame the tree grows toward), with a small sun/light tag "${lightLabel}".`;
  }

...
// LLM이 분재 공통 스키마(identity/branches/fruits/roots/light)로 구조화한다.
//
...
export const BONSAI_SCHEMA = {
  ...
  required: ["id", "displayName", "identity", "branches", "fruits", "roots", "light", "lightStrength", "character"],
  properties: {
    ...
    branches: {
      type: "array",
      maxItems: 6,
      description: "빛을 향해 뻗은 옆 역량(곁다리로 키운 것). 2~4개.",
      items: {
        type: "object",
        additionalProperties: false,
    ...
    light: {
      ...
      maxLength: 40,
      description:
        "향해 자라는 환경/시장/조직을 가리키는 짧은 명사구 하나(예: '스타트업', '헬스케어 시장', '대기업'). 여러 개를 나열하지 말고 가장 대표적인 하나만. 가지·뿌리에 쓴 역량/성과를 되풀이하지 말 것.",
    },
    lightStrength: {
      type: "integer",
      minimum: 1,
      maximum: 10,
      description:
        "향하는 목표/시장이 얼마나 크고 그 지향이 얼마나 강·명확한가(= 빛이 얼마나 강하고 멀리 뻗는지). 1~2=막연·안주, 4~6=보통, 8~10=뚜렷하고 큰 목표를 향해 내달림.",
    },
    character: {
      type: "object",
      additionalProperties: false,
      required: ["divergence", "motto"],
      properties: {
        divergence: {
          type: "integer",
          minimum: 1,
          maximum: 10,
          description:
            "직무·도메인·활동이 서로 얼마나 다른가(= 줄기가 얼마나 휘고 가지가 얼마나 벌어지는지). 1~2=한 우물만 판 외길, 4~6=인접 분야로 확장, 8~10=서로 무관한 분야를 여러 번 전전(전직·피벗).",
        },
...
- branches(가지) = 빛을 향해 뻗은 옆 역량. 곁다리로 키운 것들 (굴광성)
- fruits(열매) = 외부에서 검증된 성과. 남이 인정한 결과물 (수상/별/자격/출간/채택 등)
- roots(뿌리) = 파고든 깊이. 본인이 전문적으로 깊이 들어간 영역
- light(빛) = 향해 자라는 프레임. 환경/시장/조직을 가리키는 짧은 명사구(예: 스타트업, 헬스케어 시장). 가지·뿌리의 역량을 문장으로 되풀이하지 말 것.

규칙:
- weight는 1~10. 직군 전용 단위(별 수, 글 수, 수상 횟수…)를 1~10 상대값으로 환산해라. 가장 비중 큰 항목이 9~10, 작은 게 3~4.
- divergence(1~10)는 "직무·도메인·활동이 서로 얼마나 다른가"다(줄기 휨·가지 펼침의 양을 정함). 한 분야만 깊게=1~2, 인접 분야로 곁가지 확장=4~6, 서로 무관한 분야를 여러 번 전전=8~10.
- lightStrength(1~10)는 향하는 목표/시장을 향한 지향이 얼마나 강·명확한가다(빛의 강도·뻗음). 막연·안주=1~3, 보통=4~6, 뚜렷한 큰 목표로 내달림=8~10.
- role과 light는 여러 개를 나열하지 말고 가장 대표적인 하나만 골라라(슬래시·쉼표 나열 금지). 길면 잘려서 글자가 깨진다.
